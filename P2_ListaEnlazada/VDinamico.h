...
/**
 * @brief Exportar vector a CSV a la carpeta de proyecto.
 * @pre El tipo T debe de tener implementador operator<<.
 * @throws std::domain_error si el vector esta vacio
 */
template<class T>
void VDinamico<T>::exportaCSV(){
    if(taml == 0 || tamf == 0) throw std::domain_error("[!] VDinamico<T>::ExportaCSV: Vector vacio");
    std::ofstream fich;
    fich.open("VectorCSV.csv");
