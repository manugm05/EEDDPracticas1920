...
itirenario(), dni(_dni), pass(_pass), nombre(_nombre), direccion(_direccion), posicion(_latitud, _longitud) {
}
...
void Cliente::desbloquearMoto(Moto &m) {
    ...
    unsigned int anio = 2019 + rand() % (980);
...
void Cliente::terminarTrayecto() {
    ...
    fin = UTM(dato2, dato3);
    itirenario.fin().SetFin(fin);
    itirenario.fin().SetMinutos(rand() % 61);
    acceso->localizaMoto(this)->setPosicion(fin);
    acceso->localizaMoto(this)->seDesactiva();
}
...
ListaDEnlazada<Itinerario> Cliente::getListaIti() const {
    return itirenario;
}
